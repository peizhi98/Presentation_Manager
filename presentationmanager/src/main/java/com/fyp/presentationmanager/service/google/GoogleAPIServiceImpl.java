...
public class GoogleAPIServiceImpl implements GoogleAPIService {
    ...
    @Override
    public String addOrUpdateGoogleCalendarEvent(CalendarEventModel calendarEventModel) {
        try {
...
//            event = service.events().insert("PresentationManager0@gmail.com", event).execute();
    ...
    @Override
    public PresentationModel addOrUpdateGoogleCalendarEvent(PresentationModel presentationModel, String scheduleTitle) {
        try {
            // Build a new authorized API client service.
            final NetHttpTransport HTTP_TRANSPORT = GoogleNetHttpTransport.newTrustedTransport();
            Calendar service = new Calendar.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(HTTP_TRANSPORT))
                    .setApplicationName(APPLICATION_NAME)
                    .build();
            String location = (presentationModel.getRoomName() != null) ? presentationModel.getRoomName() : "";
            Event event;
            ...
            DateTime endDateTime = new DateTime(presentationModel.getEndTime().getTime());
            EventDateTime end = new EventDateTime()
                    .setDateTime(endDateTime)
                    .setTimeZone("Asia/Kuala_Lumpur");
            event.setEnd(end);
...
//                    eventAttendeeList.add(new EventAttendee().setEmail("wif180064@siswa.um.edu.my"));
            ...
            //google meet
            if (presentationModel.getRoomName() != null &&
                    presentationModel.getRoomName().equals("Online")) {
                ConferenceSolutionKey conferenceSKey = new ConferenceSolutionKey();
//            conferenceSKey.setType("eventHangout"); // Non-G suite user
                conferenceSKey.setType("hangoutsMeet");
                CreateConferenceRequest createConferenceReq = new CreateConferenceRequest();
...
//            event = service.events().insert("PresentationManager0@gmail.com", event).execute();
            ...
            System.out.printf("Event created: %s\n", event.getHtmlLink());
            presentationModel.setCalendarId(event.getId());
            presentationModel.setCalendarHtmlLink(event.getHtmlLink());
            if (event.getConferenceData() != null) {
                presentationModel.setGoogleMeetLink("meet.google.com/" + event.getConferenceData().getConferenceId());
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(
                    "Failed to generate google calendar event for presentation id: "
                            + presentationModel.getId());
        }
